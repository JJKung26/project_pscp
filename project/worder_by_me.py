...
def leader_bord(name, score, win, lose):#ฟังก์ชันบันทึกคะแนนผู้เล่น
    ...
    print(f"\n🎯สรุปผลของ {name}: คะแนนรวม {latest_score} | {win_lose}")
    print(f'🏅อันดับของคุณคือ {data.index(next(p for p in data if p["name"] == name)) + 1} จาก {len(data)} คน')

    # print leaderboard
    print("\n🏆 LEADERBOARD TOP 5 🏆".center(60))
    print("="*60)
    for i, player in enumerate(data[:5], start=1):
            print(f"{i}. {player['name']:<6} | Score: {player['score']:<6} | Win: {player['win']:<3} | Lose: {player['lose']:<3}")
    print('=' * 60)
    #เซฟ
    with open(filename, 'w', encoding='utf-8') as f:
...
def game_play(random_word, random_category, level,point_rate):#ฟังก์ชันเล่นเกม
        ...
        if player_round == 5: #ถ้าถึงรอบที่5แล้ว
            ...
            while True:
                ...
                if level == 'medium':#ถ้าโหมดกลาง
                    user_input = input("💡 ต้องการใบ้หมวดหมู่ไหม? (แลกกับ 1 คะแนน) [yes/no]: ")#ถามว่าต้องการใบ้ไหม
    ...
    print(f"🎯 จำนวนรอบที่ทาย: {player_round}/{round_limit}")
    print(f"💰 คะแนนที่ได้: {player_point * point_rate[level]}")
    print("="*40)
...
def main():#รอไปก่อนเดี๋ยวมาทำ
    ...
    while True:
        ...
        leader_bord(username, (userpoint * MULTIPLIER[level]), win, lose)#บันทึกชื่อและคะแนนผู้เล่น
        while True:
